...
class RLHFDataset(Dataset):
    ...
    def __getitem__(self, item):
        try:
            ...
            # Fix bug for too long prompt temporily, when come to multiturn multimodal rl may need to fix this
            if "sequence_length" in str(e) and "is larger than" in str(e):
                logger.warning(f"Skip sample idx={item} due to overlong prompt: {e}")
                return self.__getitem__((item + 1) % len(self))
            else:
                raise
